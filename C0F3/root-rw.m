...
int mount_root(task_t tfp02, uint64_t kslide) {
    ...
    uint64_t rootfs_vnode = rk642(_rootnode);
    
    // read the original flags
    uint64_t v_mount = rk642(rootfs_vnode + KSTRUCT_OFFSET_VNODE_V_UN);
    uint32_t v_flag = rk322(v_mount + KSTRUCT_OFFSET_MOUNT_MNT_FLAG + 1);
    
    // unset rootfs flag
    wk322(v_mount + KSTRUCT_OFFSET_MOUNT_MNT_FLAG + 1, v_flag & ~(MNT_ROOTFS >> 8));
    ...
    v_mount = rk642(rootfs_vnode + KSTRUCT_OFFSET_VNODE_V_UN);
    wk322(v_mount + KSTRUCT_OFFSET_MOUNT_MNT_FLAG + 1, v_flag);
