...
#ifdef __ENVIRONMENT_IPHONE_OS_VERSION_MIN_REQUIRED__
#define close(f)
    rv = kread2(base, buf, sizeof(buf));
    if (rv != sizeof(buf)) {
        printf("failed kread2, got size: %zu \n", rv);
        return -1;
    }
    ...
    rv = kread2(kerndumpbase, kernel, kernel_size);
    // rv = kread2(kerndumpbase, kernel, kernel_size);
    if (rv != kernel_size) {
        free(kernel);
        printf("failed to kread2 kern, rv: %zu \n", rv);
        return -1;
    }
...
/*
 ...
 rv = init_kernel22(base, (argc > 1) ? argv[1] : "krnl");
 assert(rv == 0);
